...
class Metattack(BaseMeta):
    ...
    def __init__(self, model, nnodes, feature_shape=None, attack_structure=True, attack_features=False, device='cpu', with_bias=False, lambda_=0.5, train_iters=100, lr=0.1, momentum=0.9):
        ...
        for ix, nhid in enumerate(self.hidden_sizes):
            ...
            epsn = Parameter(torch.zeros(1).to(device))
            eps_velocity = torch.zeros(epsn.shape).to(device) 
            self.epsns.append(epsn)
            self.eps_velocities.append(eps_velocity)
    ...
    def inner_train(self, features, adj_norm, idx_train, idx_unlabeled, labels):
        ...
        for ix in range(len(self.hidden_sizes) + 1):
            ...
            self.epsns[ix] = self.epsns[ix].detach()
            self.epsns[ix].requires_grad = True
            self.eps_velocities[ix] = self.eps_velocities[ix].detach()
            self.eps_velocities[ix].requires_grad = True
        ...
        for j in range(self.train_iters):
            ...
            for ix, w in enumerate(self.weights):
                ...
                if self.sparse_features:
                    hidden = adj_norm @ torch.spmm(hidden, w) + torch.spmm(hidden * (1 + self.epsns[ix]), w) + b
                else:
                    hidden = (adj_norm @ hidden + hidden * (1 + self.epsns[ix])) @ w  + b
                if self.with_relu:
            ...
            eps_grads = torch.autograd.grad(loss_labeled, self.epsns, create_graph=True)
            self.eps_velocities = [self.momentum * v + g for v, g in zip(self.eps_velocities, eps_grads)]
            ...
            self.epsns = [w - self.lr * v for w, v in zip(self.epsns, self.eps_velocities)]
            if self.with_bias:
