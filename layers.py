...
class RandFeaGraph(nn.Module):
    def __init__(self, in_features, out_features):
        ...
        self.w = nn.Parameter(torch.randn(in_features, out_features // 2), requires_grad=False)
        welf.sigma = nn.Parameter(torch.ones(in_features, 1))
    def forward(self, input, adj):
        proj = torch.mm(input, self.w * self.sigma)
...
class GraphConv(nn.Module):
    ...
    def forward(self, input, adj):
        ...
        #output = torch.spmm(adj, h)
        output = torch.spmm(adj, F.normalize(h))
        output = h + F.normalize(output)
        if self.bias is not None:
...
class GraphAttConvOneHead(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout=0.6, alpha=0.2):
        ...
        # init 
        nn.init.kaiming_normal_(self.weight.data)#, gain=nn.init.calculate_gain('relu')) # look at here
        nn.init.kaiming_normal_(self.a.data)#, gain=nn.init.calculate_gain('relu'))
